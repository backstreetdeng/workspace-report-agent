...
意图可选：趋势分析|画像分析|竞品分析|机会识别|政策解读|综合分析"""
...
class IntentClassifier:
    ...
    # 舆情敏感意图
    SENTIMENT_INTENTS = ["趋势分析", "画像分析", "竞品分析", "政策解读"]
    ...
    def _fallback_parse(self, question: str, response: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 意图判断
            if any(kw in response_lower for kw in ["趋势", "趋势分析", "发展前景", "未来", "走势"]):
                intent = "趋势分析"
    ...
    def _classify_with_rules(self, question: str) -> IntentResult:
        ...
        # 意图关键词
        intent_keywords = {
            "趋势分析": ["趋势", "发展前景", "未来", "走向", "预测", "走势", "合适吗", "值得买", "划算吗", "什么时候买"],
